...
#[no_mangle]
pub extern fn rcf_cuckoofilter_free(filter: *mut rcf_cuckoofilter) {
...
/// Returns `rcf_cuckoofilter_status::RCF_NOT_SUPPORTED` if the given `filter` is a null pointer.
#[no_mangle]
pub extern fn rcf_cuckoofilter_contains(filter: *const rcf_cuckoofilter, data: u64) -> rcf_cuckoofilter_status {
    let filter = unsafe { filter.as_ref() };
    filter.map_or(rcf_cuckoofilter_status::RCF_NOT_SUPPORTED,
              |filter| if filter.contains(&data) { rcf_cuckoofilter_status::RCF_OK } else { rcf_cuckoofilter_status::RCF_NOT_FOUND })
}

/// Adds `data` to the `filter`.
///
/// Returns `rcf_cuckoofilter_status::RCF_OK` if the given `data` was successfully added to the `filter`,
/// `rcf_cuckoofilter_status::RCF_NOT_ENOUGH_SPACE` if the filter could not find a free space for it.
/// Returns `rcf_cuckoofilter_status::RCF_NOT_SUPPORTED` if the given `filter` is a null pointer.
#[no_mangle]
pub extern fn rcf_cuckoofilter_add(filter: *mut rcf_cuckoofilter, data: u64) -> rcf_cuckoofilter_status {
    let filter = unsafe { filter.as_mut() };
    filter.map_or(rcf_cuckoofilter_status::RCF_NOT_SUPPORTED,
              |filter| match filter.add(&data) {
...
/// If the given `filter` is a null pointer, this function returns 0.
#[no_mangle]
pub extern fn rcf_cuckoofilter_len(filter: *const rcf_cuckoofilter) -> usize {
    let filter = unsafe { filter.as_ref() };
    filter.map_or(0, |filter| filter.len())
}

/// Checks if `filter` is empty.
/// This is equivalent to `rcf_cuckoofilter_len(filter) == 0`
/// If the given `filter` is a null pointer, this function returns false.
#[no_mangle]
pub extern fn rcf_cuckoofilter_is_empty(filter: *const rcf_cuckoofilter) -> bool {
    let filter = unsafe { filter.as_ref() };
    filter.map_or(true, |filter| filter.is_empty())
}

/// Returns the number of bytes the `filter` occupies in memory.
/// If the given `filter` is a null pointer, this function returns false.
#[no_mangle]
pub extern fn rcf_cuckoofilter_memory_usage(filter: *const rcf_cuckoofilter) -> usize {
    let filter = unsafe { filter.as_ref() };
    filter.map_or(0, |filter| filter.memory_usage())
}
...
/// Returns `rcf_cuckoofilter_status::RCF_NOT_SUPPORTED` if the given `filter` is a null pointer.
#[no_mangle]
pub extern fn rcf_cuckoofilter_delete(filter: *mut rcf_cuckoofilter, data: u64) -> rcf_cuckoofilter_status {
    let filter = unsafe { filter.as_mut() };
    filter.map_or(rcf_cuckoofilter_status::RCF_NOT_SUPPORTED,
              |filter| if filter.delete(&data) { rcf_cuckoofilter_status::RCF_OK } else { rcf_cuckoofilter_status::RCF_NOT_FOUND })
}
